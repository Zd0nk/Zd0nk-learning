...
def main():
    ...
    # Resolve logo relative to this script file (works locally + Streamlit Cloud)
    _script_dir = os.path.dirname(os.path.abspath(__file__))
    logo_b64 = ""
    for candidate in [
        os.path.join(_script_dir, "datumly_logo_light.png"),
        "datumly_logo_light.png",
        "/mnt/user-data/uploads/datumly_logo_light.png",
    ]:
        try:
            logo_b64 = _img_b64(candidate)
            break
        except Exception:
            continue
    ...
    .app-title h1 span {{ color: var(--pink); }}
    .app-title p {{
        color: var(--dim);
        font-size: 0.9rem;
        margin: 0.25rem 0 0;
    ...
    .section-hdr .ico {{ margin-right: 0.5rem; }}
    ...
    /* ── Transfer colours ── */
    .transfer-out {{ color: {RED}; font-weight: 600; }}
    .transfer-in  {{ color: {GREEN}; font-weight: 600; }}
    .captain-pick {{ color: {GOLD}; font-weight: 700; }}
    ...
    /* ══════════════════════════════════════════════════════
       AGGRESSIVE OVERRIDES — Modern Streamlit (1.30+)
       ══════════════════════════════════════════════════════ */
    ...
    .stText, p, span, label, li {{
        color: var(--text) !important;
        font-family: 'DM Sans', sans-serif !important;
    }}

    /* Radio buttons (position filter) */
    .stRadio > div {{
    ...
    /* Tab underline removal + custom styling */
    ...
    button[kind="primary"]:hover {{
        background-color: {PINK_LT} !important;
        border: none !important;
    }}
    ...
    if logo_b64:
        ...
        st.markdown("""
        <div class="datumly-header">
            <h2 style="color:#e8eaf0;margin:0;">datum<span style="color:#e8366d;">ly</span></h2>
            <div class="datumly-subtitle">Data-driven FPL intelligence</div>
            <div class="datumly-metrics-row">xG &nbsp;·&nbsp; xA &nbsp;·&nbsp; CS% &nbsp;·&nbsp; xPts &nbsp;·&nbsp; FDR</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        <div style="text-align:center;padding:1rem 0 0.5rem;">
            <span style="font-size:1.3rem;font-weight:700;color:{TEXT};">⚙️ Settings</span>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown(f"<span style='font-weight:600;color:{TEXT};'>v5.3 xPts Model</span>", unsafe_allow_html=True)
        st.markdown("""
    ...
    if not run_btn:
        ...
        left_instr, right_instr = st.columns([1, 3])
        with left_instr:
            st.markdown(f"""
            <div class="instructions">
                <h4>👥 Instructions</h4>
                <ol>
                    <li>Enter your FPL Manager ID.</li>
                ...
                <p style="color:var(--dim);font-size:0.78rem;margin-top:0.75rem;">
                    Need help? Open the sidebar for docs.
                </p>
            </div>
            """, unsafe_allow_html=True)
        with right_instr:
    ...
    <div class="stat-cards">
        <div class="stat-card">
            <span class="stat-icon" style="color:{PINK};">↑</span>
    ...
    # ── Tabbed content area ─────────────────────────────────
    tab_squad, tab_fixtures, tab_insights = st.tabs([
        "👥 Your Squad ⚽",
        "Fixture Difficulty 📅",
        "Insights 📊",
    ])
    ...
    # ── TAB: Your Squad ─────────────────────────────────────
    with tab_squad:
        show_cols = ["web_name","team_short","price","sell_price","form",
                     "xPts","cs_prob","xg_p90","xa_p90","dc_hit_rate",
                     "ownership","fpl_value_score","rotation_risk"]
        show_cols = [c for c in show_cols if c in my_team_df.columns]

        nice_names = {
            "web_name": "Player", "team_short": "Team", "price": "Price",
            "sell_price": "Sell", "form": "Form", "xPts": "xPts",
            "cs_prob": "CS%", "xg_p90": "xG/90", "xa_p90": "xA/90",
            "dc_hit_rate": "DC%", "ownership": "Own%",
            "fpl_value_score": "Score", "rotation_risk": "Risk",
        }
        ...
        if pos_filter == "ALL":
            ...
            filtered = my_team_df[my_team_df["position"] == pos_filter]

        if not filtered.empty:
            display = filtered[show_cols].copy()
            display = display.rename(columns={c: nice_names.get(c, c) for c in display.columns})
            # Format numeric columns
            for col in ["Price", "Sell", "Form", "xPts", "Score"]:
            ...
            for col in ["xG/90", "xA/90"]:
                if col in display.columns:
                    display[col] = display[col].apply(lambda x: f"{x:.2f}" if pd.notna(x) else "—")
            if "Own%" in display.columns:
                display["Own%"] = display["Own%"].apply(lambda x: f"{x:.1f}%" if pd.notna(x) else "—")

            st.dataframe(display, use_container_width=True, hide_index=True)
        else:
            st.info("No players in this position.")

    # ── TAB: Fixture Difficulty ─────────────────────────────
    with tab_fixtures:
        st.markdown(f'<div class="section-hdr"><span class="ico">📅</span>Fixture Difficulty — GW{active_gw+1} to GW{active_gw+horizon_gws}</div>', unsafe_allow_html=True)

        fs = fix_df.sort_values("avg_difficulty")
        col_easy, col_hard = st.columns(2)
        with col_easy:
            st.markdown(f"**🟢 Easiest Runs**")
            easy = fs[["name","avg_difficulty","num_fixtures","has_dgw"]].head(8).copy()
            easy.columns = ["Team","Avg Diff","Fixtures","DGW"]
            easy["Avg Diff"] = easy["Avg Diff"].apply(lambda x: f"{x:.2f}")
            st.dataframe(easy, use_container_width=True, hide_index=True)
        with col_hard:
            st.markdown(f"**🔴 Hardest Runs**")
            hard = fs[["name","avg_difficulty","num_fixtures","has_dgw"]].tail(8).copy()
            hard.columns = ["Team","Avg Diff","Fixtures","DGW"]
            hard["Avg Diff"] = hard["Avg Diff"].apply(lambda x: f"{x:.2f}")
            st.dataframe(hard, use_container_width=True, hide_index=True)

    # ── TAB: Insights ───────────────────────────────────────
    with tab_insights:
        # DC leaders
        st.markdown(f'<div class="section-hdr"><span class="ico">🛡️</span>Top DC Earners (last {dc_lookback} GWs)</div>', unsafe_allow_html=True)
        ...
        for col_target, (pos, label) in zip([dc_col1, dc_col2],
            ...
            with col_target:
                ...
                dc_display = sub[dc_show].copy()
                dc_display.columns = [c.replace("_"," ").title() for c in dc_display.columns]
                st.dataframe(dc_display, use_container_width=True, hide_index=True)

        # xG Source Comparison
        st.markdown('<div class="section-hdr"><span class="ico">📊</span>xG Source Comparison — Understat vs FBref</div>', unsafe_allow_html=True)
        st.caption(
            "Two independent xG models compared. "
            "Large discrepancies → treat projections with more caution. "
            "The optimizer uses a blended average."
        )
        ...
        if not has_both.empty and len(compare_cols) > 6:
            xg_tab_disc, xg_tab_squad = st.tabs(["⚠️ Biggest Discrepancies", "👥 Your Squad"])

            col_labels = [
                "Player", "Team", "Pos", "Price",
                "US xG/90", "FB xG/90", "xG Gap",
                "US xA/90", "FB xA/90", "xA Gap",
                "Blend xG/90", "Blend xA/90"
            ]

            with xg_tab_disc:
                top_disc = has_both.sort_values("xg_discrepancy", ascending=False).head(15)
                disc_display = top_disc[compare_cols].copy()
                disc_display.columns = col_labels[:len(compare_cols)]
                st.dataframe(disc_display, use_container_width=True, hide_index=True)

            with xg_tab_squad:
                squad_compare_cols = [c for c in compare_cols if c in my_team_df.columns]
                if squad_compare_cols:
                    squad_display = my_team_df[squad_compare_cols].copy()
                    squad_display.columns = col_labels[:len(squad_compare_cols)]
            ...
            sc1.metric("Understat", f"{us_count} players")
            sc2.metric("FBref", f"{fb_count} players")
            sc3.metric("Both sources", f"{both_count} players")
        else:
            st.info("FBref data unavailable — using Understat only.")

    # ── Rolling transfer plan ───────────────────────────────
    st.markdown(f'<div class="section-hdr"><span class="ico">📋</span>Rolling {horizon_gws}-GW Transfer Plan (GW{next_gw} → GW{next_gw + horizon_gws - 1})</div>', unsafe_allow_html=True)
