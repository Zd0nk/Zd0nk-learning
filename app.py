...
========================================================================
v5.5 changes:
  - Removed ep_next (FPL's expected points) from scoring entirely.
    xPts is now 100% driven by Understat/FBref underlying stats
    (xG, xA, CS prob, DC, bonus, saves) + FPL scoring rules.
  - Injury/availability captured by start_prob and risk flags instead.
  - Transfer cards now show xPts on both sides (no more ep: metric).
  - Per-90 bonus/saves/YC/GC use stabilised denominator (min 450 mins)
    to prevent small-sample inflation.
...
Data sources (all free, no auth):
  - FPL API    → squad, prices, form, GW history, DC stats
...
def compute_expected_pts(df):
    ...
    v5.5: xPts is purely stat-driven (Understat + FBref). No ep_next
    blending — injury/rotation is handled by start_prob and risk flags.

    Per-90 bonus/saves/YC/GC use stabilised denominator (min 450 mins)
    to prevent small-sample inflation for low-minute players.
...
"""
    ...
    <div class="datumly-header">
        ...
        <div class="datumly-subtitle"></div>
    </div>
    ...
    with input_col:
        fpl_id = st.number_input(
            "FPL Manager ID:",
            min_value=1, value=123456789, step=1,
        ...
        with st.expander(
            ...
            if not gw_plan["transfers"]:
                ...
                for i, t in enumerate(gw_plan["transfers"], 1):
                    ...
                    with c1:
                        st.markdown(f'''<span class="transfer-out">⬇ OUT: {t["OUT"]}</span> [{t["OUT_pos"]}] sell £{t["OUT_sell_price"]:.1f}m (mkt £{t["OUT_buy_price"]:.1f}m)<br>xPts: {t["OUT_score"]:.2f} · DC: {t["OUT_dc_hit_rate"]:.0%} · fix: {t["OUT_fix_diff"]:.2f} · {t["OUT_risk"]}''', unsafe_allow_html=True)
                    with c2:
                        st.markdown("### →")
                    with c3:
                        st.markdown(f'''<span class="transfer-in">⬆ IN: {t["IN"]}</span> [{t["IN_pos"]}] ({t["IN_team"]}) £{t["IN_price"]:.1f}m<br>**xPts: {t["IN_xPts"]:.2f}** · xG: {t["IN_xg_p90"]:.3f} · xA: {t["IN_xa_p90"]:.3f} · DC: {t["IN_dc_hit_rate"]:.0%} · CS: {t["IN_cs_prob"]:.0%}<br>**Score gain: {t["score_gain"]:+.3f}** · Price diff: {t["price_diff"]:+.1f}m''', unsafe_allow_html=True)
